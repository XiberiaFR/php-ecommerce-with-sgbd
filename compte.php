...
if (!empty($_POST)) {
    if (empty($_POST['password']) || $_POST['password'] != $_POST['password_confirm']) {
        ...
        if (password_verify($_POST['password_old'], $user['mot_de_passe'])) {
            ...
            $pdo->prepare('UPDATE clients SET mot_de_passe = ?')->execute([$password]);
            echo "<div class='alert alert-success'>Votre mot de passe a bien été mis à jour</div>";
        }
        else {
            echo "<div class='alert alert-danger'>Le mot de passe que vous venez de taper comme étant le mot de passe actuel ne correspond pas au mot de passe actuellement enregistré sur votre compte</div>";
        }
    }
}
?>
